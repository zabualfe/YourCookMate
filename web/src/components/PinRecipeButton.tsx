...
export function PinRecipeButton({
  ...
  const className = pinned
    ? compact
      ? 'inline-flex min-h-9 items-center gap-1.5 rounded-xl border border-brand-200 bg-brand-50 px-3.5 py-2 text-sm font-medium text-brand-800 hover:bg-brand-100 disabled:opacity-60'
    ...
    : compact
      ? 'inline-flex min-h-9 items-center gap-1.5 rounded-xl border border-stone-200 px-3.5 py-2 text-sm font-medium text-stone-700 hover:bg-stone-50 disabled:opacity-60'
  ...
  return (
    <span className={`inline-flex flex-col gap-1 ${compact ? 'items-start' : 'items-end'}`}>
      <button
        type="button"
      ...
      {error && (
        <span className={`max-w-[12rem] text-xs text-red-700 ${compact ? 'text-left' : 'text-right'}`}>
          {error}
        </span>
      )}
    </span>
